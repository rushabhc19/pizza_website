...
function get_preparing_orders_by_room($db, $room) {
...
function get_baked_orders_by_room($db, $room) {
...
function add_order($db, $room,$size,$current_day,$status, $topping_ids) {
    $query = 'INSERT INTO pizza_orders(room_number, size, day, status) '
            . 'VALUES (:room,(select size from menu_sizes where id = :size),:current_day,:status)';
    $statement = $db->prepare($query);
    $statement->bindValue(':room',$room);
